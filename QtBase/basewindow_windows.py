...
class BaseWindow_Windows(QWidget):
    ...
    def __init__(self, widget) -> None:
        ...
        if self.widget.parent() != None:
            self.resize(widget.width(), widget.height() + self.TH)
        else:
...
QPushButton#close_button:hover{
    ...
    def resizeEvent(self, a0) -> None:
        self.widget.resize(self.width(), self.height()-self.TH)
        self.widget.move(0, self.TH)
        ...
        try:
            if pos.x() < 0 and pos.y() < 0:  # 防止超出屏幕(一般发生在最大化的时侯)
                ...
                if self.width() >= width and self.height() >= height:
                    self.maxnormal_button.setIcon(
                        qta.icon('fa5.window-restore'))
                    # 得到偏移的大小
                    x = 0-pos.x() if pos.x() < 0 else 0
                    y = 0-pos.y() if pos.y() < 0 else 0
                    self.YS = y
                    # moveEvent比resizeEvent晚发出,所以不会影响
                    self.widget.move(x, y+self.TH)
                    self.widget.resize(width, height-self.TH)
                    self.title.move(x, y)
